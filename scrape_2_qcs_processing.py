...
class Processing:
    ...
    def make_final_df(self):
        ...
        invoice_name=df.loc[1,'Unnamed: 5']          # Invoice Name
        # find_order_no=False
        # find_service=False
        # find_qty=False
        ...
        df2[invoice_name]=0
        df2['fsa_id']=df2.fsa_id.apply(str)
        df2['service_code']= df2.service_code.apply(str)
        col1=df.columns[0]
        for i in range(len(df)):
            if df.loc[i,col1]=="S No.":
                start_row=i
                break
        # print(df2.loc[0,])
        for i in range(start_row+1,len(df)):
            # print(i)
            if type(df.loc[i,col1])!=int:
                break
            else:
                fsa=str(df.loc[i,'Unnamed: 1'])
                # print(fsa)
                fsa_id_no=str(re.findall(r'\d+',fsa)[0])
                df3= df2[df2['fsa_id']==fsa_id_no]
                # print(df3)
                ser_code=str(df.loc[i,'Unnamed: 2'])
                # print(ser_code)
                df4=df3[df3['service_code']==ser_code]
                n=list(df4['index'])
                # print(n)
                n=n[0]
                qty=df.loc[i,'Unnamed: 5']
                df2.loc[n,invoice_name]=qty
